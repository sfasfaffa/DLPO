...
from .optimizer_prompts import construct_tgd_prompt, OPTIMIZER_SYSTEM_PROMPT, GRADIENT_TEMPLATE, GRADIENT_MULTIPART_TEMPLATE,construct_tgd_promptV2
...
class TextualGradientDescentV2(Optimizer):
    def __init__(self, 
                 parameters: List[Variable], 
                 verbose: int=0, 
                 engine: Union[EngineLM, str]=None, 
...
2. List key data
3. Show calculation steps
4. Analyze complexity [adding sentence]
5. Assess stability [adding sentence]
...
#  by updating the most important ones and discarding the relatively unimportant ones.
        ...
        # else:
        self.optimizer_system_prompt = optimizer_system_prompt.format(new_variable_start_tag=new_variable_tags[0], new_variable_end_tag=new_variable_tags[1]) + self.lr_prompt
        # print(self.optimizer_system_prompt)
        self.do_constrained = (len(self.constraints) > 0)
        self.new_variable_tags = new_variable_tags
        self.in_context_examples = in_context_examples if in_context_examples is not None else []
